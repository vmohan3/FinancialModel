...
/**
 * This is the implementation of perceptron predictor algorithm for the project.
...
public class Perceptron 
{
	...
	public static void train(List<Instance> instances)
	{
		...
		beta=0.0;
		double online_learning_rate=1;
		double wixi=0.0;
		int online_training_iterations=200;
		int predicted_label;
		n=calc_n(train_inst);
		w=new double[n];
			...
			for(int j=0;j<len;j++)
			{
				...
				if(predicted_label!=origy)
				{
					for(int k=0;k<n;k++)
					{
						...
						w[k]=w[k]+online_learning_rate*x*origy;
					}
				}
				wixi=0.0;
			}
